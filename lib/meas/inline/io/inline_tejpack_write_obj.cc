...
namespace Chroma
{
  namespace InlineTejPackWriteNamedObjEnv
  {
    namespace
    {
      ...
      const std::string name = "TEJPACK_WRITE_NAMED_OBJECT";
    }
    ...
    bool registerAll()
    {
      ...
      {
        // Datatype writer
        success &= TejPackWriteObjCallMapEnv::registerAll();
    ...
    void read(XMLReader& xml, const std::string& path, Params::File_t& input)
    {
      ...
      /*
       *  Now information about slicing
       */
      read(inputtop, "j_decay", input.j_decay);
      read(inputtop, "t_slice", input.t_slice);
    }
			   ...
			   XMLWriter& xml_out) 
    {
      ...
      push(xml_out, "tejpack_write_named_obj");
	...
	// Write the object
	swatch.start();
	TejPackWriteObjCallMapEnv::TheTejPackWriteObjFuncMap::Instance()
            .callFunction(params.named_obj.object_type,
