...
#else
      spinorIn = QDP_get_global_cache().getDevicePtr( chi_s.getId() );
...
#else
      spinorIn = QDP_get_global_cache().getDevicePtr( mod_chi.getId() );
      QDPIO::cout << "MDAGM spinor in = " << spinorIn << "\n";
#endif
    }
    else { 
...
#else
    void* spinorOut = QDP_get_global_cache().getDevicePtr( psi_s.getId() );
